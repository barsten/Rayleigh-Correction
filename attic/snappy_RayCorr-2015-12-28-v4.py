...
raycorProduct.setAutoGrouping('rtoa:taur:rRay:rRayF1:rRayF2:rRayF3:transSRay:transVRay:sARay:rtoaRay:rBRR')
...
# gridThetaS = np.zeros(dimThetaS,  dtype=np.float32)  # todo: read Coeffcients from file
# gridThetaV = np.zeros(dimThetaV,  dtype=np.float32)  # todo: read Coeffcients from file
gridThetaS = rayADF['theta']
gridThetaV = rayADF['theta']
# RayScattCoeffA = np.zeros( (dimThetaS, dimThetaV, 3), dtype=np.float32)  # todo: read Coeffcients from file
# RayScattCoeffB = np.zeros( (dimThetaS, dimThetaV, 3), dtype=np.float32)  # todo: read Coeffcients from file
# RayScattCoeffC = np.zeros( (dimThetaS, dimThetaV, 3), dtype=np.float32)  # todo: read Coeffcients from file
# RayScattCoeffD = np.zeros( (dimThetaS, dimThetaV, 3), dtype=np.float32)  # todo: read Coeffcients from file
# dummy intitialisierung, todo: remove after reading from file
# gridThetaS = np.linspace(0., 90., num=dimThetaS)
# gridThetaV = np.linspace(0., 90., num=dimThetaV)
gridGeometry = [gridThetaS, gridThetaV]
# RayScattCoeffA.fill(1.0)
# RayScattCoeffB.fill(1.0)
# RayScattCoeffC.fill(1.0)
# RayScattCoeffD.fill(1.0)
...
# rayAlbLUT = np.zeros(dimTaur, dtype=np.float32)  # Rayleigh spherical albedo LUT todo: implement LUT
...
for y in range(height-120):
    ...
    for x in range(width):
        ...
        for j in [0, 1, 2]:
            a[j] = interpn(gridGeometry, RayScattCoeffA[:,:,j], [(theta_s[x], theta_v[x])], method='linear', bounds_error=False)
            b[j] = interpn(gridGeometry, RayScattCoeffB[:,:,j], [(theta_s[x], theta_v[x])], method='linear', bounds_error=False)
            c[j] = interpn(gridGeometry, RayScattCoeffC[:,:,j], [(theta_s[x], theta_v[x])], method='linear', bounds_error=False)
            d[j] = interpn(gridGeometry, RayScattCoeffD[:,:,j], [(theta_s[x], theta_v[x])], method='linear', bounds_error=False)
        ...
        for i in range(nbands):
            ...
            for j in [0,1,2]:
                    ...
                    if j==0:
                       rayMultiCorr[j]=1  # todo remove
                    else:
                        rayMultiCorr[j]=0  # todo remove
            ...
            tR_thetaV[(i,x)] = 1  # todo remove
            tR_thetaS[(i,x)] = 1  # todo remove

            sARay[(i,x)] = rayAlb_f(taur[(i,x)])  # Rayleigh spherical albedo
            sARay[(i,x)] = 0.0  # todo: remove!

            rho_toaR[(i,x)] = (reflectance[(i,x)] - rho_R[(i,x)])/(tR_thetaS[(i,x)]*tR_thetaV[(i,x)])  # toa reflectance corrected for Rayleigh scattering
            rho_BRR[(i,x)] = rho_toaR[(i,x)]/(1+sARay[(i,x)]*rho_toaR[(i,x)])  # top of aerosol reflectance, which is equal to bottom of Rayleigh reflectance
